...
def test_sample(scoring_rule_approximator_any, simulator, batch_size, num_samples, adapter):
    ...
    samples_merged = scoring_rule_approximator_any.sample(num_samples=num_samples, conditions=data)
    ...
    samples_separate = scoring_rule_approximator_any.sample(
        num_samples=num_samples, conditions=data, merge_scores=False
    )
